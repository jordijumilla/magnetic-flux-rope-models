...
all_param_combinations: list = list(itertools.product(delta_range, xi_range, B_z_0_range, tau_range, C_nm, n_range, m_range))

# Define the test tolerance
test_tolerance: float = 1e-8

@pytest.mark.parametrize("delta, xi, B_z_0, tau, C_nm, n, m", all_param_combinations)
def test_B_z_0(delta: float, xi: float, B_z_0: float, tau: float, C_nm: float, n: int, m: int) -> None:
    """
    Test that the B_z_0 parameter is correctly set in the ECModel.
    For a range of delta, xi, B_z_0, and tau values, check that the z-component
    of the magnetic field at the center (0, 0) matches B_z_0 within a tight tolerance.
    """
    ec_model: ECModel = ECModel(delta=delta, xi=xi, B_z_0=B_z_0, tau=tau, C_nm=C_nm, n=n, m=m)
...
@pytest.mark.parametrize("delta, xi, B_z_0, tau, C_nm, n, m", all_param_combinations)
def test_tau(delta: float, xi: float, B_z_0: float, tau: float, C_nm: float, n: int, m: int) -> None:
    """
    Test that the tau parameter is correctly set in the ECModel.
    For a range of delta, xi, B_z_0, and tau values, check that the theoretical tau,
    ...
    ec_model: ECModel = ECModel(n=n, m=m, delta=delta, xi=xi, B_z_0=B_z_0, tau=tau, C_nm=C_nm)
...
@pytest.mark.parametrize("delta, xi, B_z_0, tau, C_nm, n, m", all_param_combinations)
def test_C_nm(delta: float, xi: float, B_z_0: float, tau: float, C_nm: float, n: int, m: int) -> None:
    """
    Test that the C_nm parameter is correctly set in the ECModel.
    For a range of delta, xi, B_z_0, and tau values, check that the C_nm value
