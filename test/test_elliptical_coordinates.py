...
xi_range = np.deg2rad([0, 22.5, 45, 67.5, 90, 112.5, 135, 157.5])

# Generate all combinations
all_param_combinations: list = list(itertools.product(R_range, delta_range, xi_range))

# Define the test tolerance
test_tolerance: float = 1e-8

@pytest.mark.parametrize("R, delta, xi", all_param_combinations)
def test_identity_change_of_coordinates_cartesian_to_elliptical(R: float, delta: float, xi: float) -> None:
    # Create an instance of the model with the given R, delta and xi.
    model = EllipticalCylindricalModel(R=R, delta=delta, xi=xi)
...
@pytest.mark.parametrize("R, delta, xi", all_param_combinations)
def test_identity_change_of_coordinates_elliptical_to_cartesian(R: float, delta: float, xi: float) -> None:
    # Create an instance of the model with the given R, delta and xi.
    model = EllipticalCylindricalModel(R=R, delta=delta, xi=xi)
...
def get_quadrant_point(R: float, delta: float, xi: float, quadrant: int) -> tuple[float, float]:
    """Get a point in the specified quadrant of the elliptical elliptical-cylindrical coordinates."""
    if quadrant == 1:
        x = R*delta*math.cos(xi)
        y = R*delta*math.sin(xi)
    elif quadrant == 2:
        x = -R*math.sin(xi)
        y = R*math.cos(xi)
    elif quadrant == 3:
...
@pytest.mark.parametrize("R, delta, xi", all_param_combinations)
def test_quadrants(R: float, delta: float, xi: float) -> None:
    # Create an instance of the model with the given delta, xi and R.
    model = EllipticalCylindricalModel(delta=delta, xi=xi, R=R)

    # Test each quadrant.
    for quadrant, expected_phi in zip([1, 2, 3, 4], [0, math.pi/2, math.pi, -math.pi/2]):
        x, y = get_quadrant_point(R=R, delta=delta, xi=xi, quadrant=quadrant)
...
@pytest.mark.parametrize("R, delta, xi", all_param_combinations)
def test_elliptical_to_cartesian(R: float, delta: float, xi: float) -> None:
    # Create an instance of the model with the given delta, xi and R.
    model = EllipticalCylindricalModel(delta=delta, xi=xi, R=R)
    ...
    for z in [-1, 0, 1]:
        for quadrant, phi in zip([1, 2, 3, 4], [0, math.pi/2, math.pi, 3*math.pi/2]):
            ...
            expected_coordinates = get_quadrant_point(R=R, delta=delta, xi=xi, quadrant=quadrant)
...
@pytest.mark.parametrize("R, delta, xi", all_param_combinations)
def test_elliptical_unit_basis_scalar(R: float, delta: float, xi: float) -> None:
    # Create an instance of the EllipticalCylindricalModel with specific parameters.
    ec_model = EllipticalCylindricalModel(R=R, delta=delta, xi=xi)
    ...
    assert np.allclose(unit_basis[:, 0], np.array([np.cos(ec_model.xi), np.sin(ec_model.xi), 0]), atol=1e-8)
    assert np.allclose(unit_basis[:, 1], np.array([-np.sin(ec_model.xi), np.cos(ec_model.xi), 0]), atol=1e-8)
